...
class TreeCropTypesDescription extends Db {
    ...
    public function findCropTreeDescription($where = "") {
        $table = self::$table_name . " JOIN tbl_tree_or_crop_types ON tree_crop_id = tbl_tree_or_crop_types.id "
                . "JOIN tbl_crop_description ON crop_description_id = tbl_crop_description.id";
