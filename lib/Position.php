...
class Position extends Db {
	...
	public function findPositionDetails(){
		$result_array = $this->getfarray(self::$table_name." p, accesslevel a", "p.id, p.title,p.description, a.name as access_level, a.id as access_level_id", "p.access_level = a.id AND p.active=1 ",  "", "");
		return !empty($result_array) ? $result_array : false;
	}
