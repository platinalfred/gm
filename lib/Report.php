...
class Report extends Db {
	...
	public function getProjectReport($where = 1){
		//teh crops/trees
		$crop_fields = "`tbl_pap_crop_tree`.`id`", "`pap_id`", "`quantity`", "`tbl_pap_crop_tree`.`rate` `crop_rate`", "`crop_description_rate_id`", "`croptype`", "`cropdescription`";
		...
		//the improvements
		$improvements_fields = "`tbl_pap_improvement`.`id`", "`pap_id`", "`quantity`", "`tbl_pap_improvement`.`rate`" "`improvement_rate`", "`district_property_rate_id`", "`propertytype`", "`propertydescription`";
		...
		$fields = "`tbl_paps`.`id`, `project_id`, `pap_ref`, `firstname`, `othername`, `lastname`, `phone_contact`, `district_id`, `district_name`, `county_id`, `county_name`, `subcounty_id`, `subcounty_name`, `parish_id`, `parish_name`, `village_id`, `village_name`, `way_leave`, `rightofway`, `total_take`, `chainage`, `crop_tree_cnt`, `crop_tree_sum`, `improvement_cnt`, `improvement_sum`, ".implode(',',array_slice($crop_fields, 2)).", ".implode(',',array_slice($improvements_fields, 2));
